...
class addcoin(commands.Cog):
    ...
    async def addcoin(self, ctx, uuid):
        ...
        try:
            if useage == "true":
                embed = discord.embeds.Embed(title="MZ Point", description="등록실패", color=0xFF0000)
                ...
                try:
                    ...
                    for document in await cursor2.to_list(length=100):
                        useramount = document["point"]
                    embed = discord.embeds.Embed(title="MZ Point", description="등록완료", color=0x88FF00)
                    embed.add_field(name="현제 보유중인 코인", value=f"{useramount}MZ")
                    embed.set_footer(text="shop.audiscordbot.xyz")
                    await ctx.send(embed=embed)
                except UnboundLocalError:
                    await db.AUpoint.insert_one(
    ...
    async def infocoin(self, ctx):
        try:
            ...
            for document in await cursor2.to_list(length=100):
                useramount = document["point"]
            embed = discord.embeds.Embed(title="MZ Point", description="정보", color=0x88FF00)
            embed.add_field(name="현제 보유중인 코인", value=f"{useramount}MZ")
            embed.set_footer(text="shop.audiscordbot.xyz")
            await ctx.send(embed=embed)
        except UnboundLocalError:
            embed = discord.embeds.Embed(title="MZ Point", description="정보", color=0x88FF00)
            embed.add_field(name="현제 보유중인 코인", value=f"0MZ")
            embed.set_footer(text="shop.audiscordbot.xyz")
            await ctx.send(embed=embed)
        except Exception as e:
            embed = discord.embeds.Embed(title="MZ Point", description="정보", color=0x88FF00)
